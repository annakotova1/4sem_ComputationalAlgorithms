...
# Хрень какая то эти две функции
def get_legandr_roots(x, n):
    p = [0, x]
    for i in range(2, n + 1):
        p.append(recurrent_formula(x, i + 2))
    return p

def recurrent_formula(x, n):
    if (n == 0):
        return 1
    if (n == 1):
        return x
    p_current = 1/n*((2*n - 1)* x * recurrent_formula(x, n - 1) - (n - 1) * recurrent_formula(x, n - 2))
    return p_current

# Попытка 2
from sympy import diff, symbols, lambdify
def legrange(x, n):
    return make_diff(diff((x*x - 1)**n), n)
def make_diff(x, n):
    if (n == 1):
        return x
    return make_diff(diff(x), n - 1)
